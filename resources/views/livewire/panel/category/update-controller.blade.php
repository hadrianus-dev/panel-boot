                            ...
                            <div class="row">
                                <div class="col-xl-12">
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-lg-10">
                                            <select wire:model='data.parent' class="default-select wide form-control" id="validationCustom05">
                                                <option  data-display="Selecionar">Selecionar</option>
                                                @foreach ($categoryParent as $item)
                                                @if ($item->id == $categoryData['parent'])
                                                <option selected value="{{$item->id}}" wire:click="$emit('catEdit', {{$item->id}})">{{$item->title}}</option>
                                                @else
                                    ...
                                    <div class="mb-3 row">
                                        ...
                                        <div class="col-lg-10">
                                            <select wire:model='data.published' class="default-select wide form-control" id="validationCustom05">
                                                <option  data-display="{{($categoryData['published'] == true) ? 'Publicado' : 'Rascunho'}} ">Selecionar</option>
                                                <option value='1' wire:click="$emit('status', 1)">Publicar</option>
                                                <option value='0' wire:click="$emit('status', 2)">Rascunho</option>
                                            </select>
                                    ...
                                    <div class="text-right">
                                        <button type="button" wire:click.prevent="update" class="btn btn-primary">Salvar<span
